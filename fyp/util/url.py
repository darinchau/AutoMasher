...
def download_audio_with_yt_dlp(link: YouTubeURL, output_dir: str, port: int | None = None, verbose=True):
    try:
    ...
    for file in os.listdir(output_dir):
        if link.video_id in file and file.endswith('.wav'):
            return os.path.join(output_dir, file)
    raise FileNotFoundError(f"Could not find downloaded file for {link}: {retcode}")


def download_audio(link: YouTubeURL, output_dir: str, verbose=True, timeout=120, port: int | None = None):
    return download_audio_with_yt_dlp(link, output_dir, verbose=verbose, port=port)
